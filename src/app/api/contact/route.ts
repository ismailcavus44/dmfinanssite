...
export async function POST(request: NextRequest) {
  try {
    ...
    const { firstName, lastName, phone, retirementStatus, message } = body;

    // Form validasyonu
    if (!firstName || !lastName || !phone || !retirementStatus || !message) {
    ...
    // Emeklilik durumu Türkçe metin
    const retirementStatusText = retirementStatus === 'emekliyim' ? 'Emekliyim' : 'Emekli değilim';
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="background-color: #f9fafb; padding: 20px; border-radius: 8px; margin-top: 20px;">
            ...
            <p><strong>Emeklilik Durumu:</strong> ${retirementStatusText}</p>
          </div>
