...
export default function Header() {
  ...
  const revealOnScroll = pathname === "/" || pathname === "/iletisim" || pathname === "/hakkimizda";

  return (
    <header 
      className={
        revealOnScroll
          ? `fixed top-0 left-0 right-0 z-50 border-b border-gray-200 bg-white/95 backdrop-blur-sm shadow-sm transition-all duration-500 ease-in-out ${
              isScrolled ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0'
            }`
          : "fixed top-0 left-0 right-0 z-50 border-b border-gray-200 bg-white/95 backdrop-blur-sm shadow-sm"
      }
    >
